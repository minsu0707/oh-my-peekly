...
- 이슈로 판단한 화면마다:
  ...
  2. `AskUserQuestion` 도구로 물어본다:
     - 질문 본문에 `[n/총계] {경로 브레드크럼} --> [{계정/권한}]`과 `문제: {발견한 문제 설명}`을 포함한다.
     - 옵션: `["포함 (권장)", "제외"]`.
  3. "포함"이 기본/권장 옵션이다. 이 단계는 AI 오탐(false positive)을 걸러내는 안전장치이므로, 사용자가 "제외"를 선택한 이슈만 이후 보고서 생성 대상에서 뺀다.

## 6단계 — 보고서 생성

1. "포함"으로 확정된 이슈만 필터링한다.
