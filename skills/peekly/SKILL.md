---
description: 서비스 URL/계정/PPT 템플릿만 있으면 웹 서비스를 순회하며 QA 테스트를 수행하고, 이슈를 사용자 검수(Y/N) 후 계정별 PPT 보고서로 생성한다.
...
3. 사용자에게 다음 형태로 안내한다:
   ```
   사이트맵 크롤링 중... 총 {totalCount}개 화면 발견
   예상 소요 시간: 약 {estimatedMinutes}분 / 예상 비용: 약 {estimatedCostKrw}원
   진행하시겠습니까? (Y/n):
   ```
   - `truncated: true`이면 크롤링이 상한(도구의 provisional 기본값, 현재 50페이지)에서 잘렸을 수 있다는 점도 함께 알려라.
4. **사용자가 명시적으로 승인(Y 또는 Enter)해야만** 3단계로 진행한다. `n`이면 아무 것도 실행하지 않고 즉시 종료한다.
...
- "정상"으로 판단한 화면은 이 검수 목록에 **아예 노출하지 않는다** (질문 대상에서 제외).
- 이슈로 판단한 화면마다:
  ...
  2. 다음 형태로 물어본다:
     ...
     문제: {발견한 문제 설명}
     스크린샷을 엽니다...
     이 이슈를 보고서에 포함할까요? (Y/n):
     ```
  3. Enter만 눌러도 기본값(Y=포함)으로 진행된다. 이 단계는 AI 오탐(false positive)을 걸러내는 안전장치이므로, 사용자가 N을 선택한 이슈는 이후 보고서 생성 대상에서 제외한다.
...
4. 파일명은 사용자에게 먼저 물어본다: "저장할 파일명을 입력하세요 (Enter 시 기본값 사용)".
   - 기본값: `템플릿이름_계정명_날짜`
   - 사용자가 직접 이름을 입력한 경우: 계정이 1개면 그 이름 그대로 사용, 계정이 여러 개면 사용자가 입력한 이름 뒤에 계정명을 자동으로 붙여 계정별로 구분한다. **이 다중 계정 파일명 분기 로직은 `generate_report` 도구가 하지 않으므로, 반드시 네가 계정별 `outputPath`를 직접 계산해서 넘겨야 한다.**
