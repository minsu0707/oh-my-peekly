...
let contextInstance: BrowserContext | undefined;
const pages = new Map<string, Page>();
let cleanupRegistered = false;

async function getPage(sessionId: string = DEFAULT_SESSION_ID): Promise<Page> {
  if (!browserInstance) {
...
const sessionIdField = z
  ...
  .optional()
  .describe(
    `Which browser tab/session to act on. Omit to use the default single-tab session (${DEFAULT_SESSION_ID}) — ` +
...
export function registerBrowserTools(server: McpServer): void {
  ...
  server.registerTool(
    "browser_new_session",
    {
      ...
      description:
        "Open a new tab (Playwright page) within the same shared browser context, so it shares login/cookies " +
  ...
  server.registerTool(
    "browser_close_session",
    {
      ...
      description:
        "Close a tab/session previously opened with browser_new_session, freeing its resources. Closing the " +
