...
2. 테스트결과서 PPT 템플릿은 **파일명만** 물어본다(전체 경로 아님). `find_local_file(filename)`을 호출해 로컬에서 찾는다.
   ...
   - **이 템플릿 파일은 끝까지 읽기 전용이다.** `generate_report`는 이 경로를 참고해서 항상 별도의 새 파일(`outputPath`)을 만들고 그 새 파일만 수정한다 — 템플릿 원본을 열어서 고치거나 덮어쓰는 일은 없다(도구 자체가 `outputPath == templatePath`이면 에러로 거부한다). 6단계에서 파일명을 "직접 입력"으로 받을 때도 템플릿과 같은 이름/경로를 쓰지 않도록 주의해라.
...
6. **병렬 테스트 세션 수**: 이 항목은 다른 설정과 달리 **캐시하지 않고 실행할 때마다 매번 묻는다** (사용자가 실행마다 상황에 따라 다르게 고르고 싶어함 — 예: 대상 서비스 부하 상태에 따라). `get_setting(key: "parallelSessionCount")`로 직전에 골랐던 값이 있는지만 확인해서 `AskUserQuestion`의 `(권장)` 라벨을 그 값에 맞게 붙이되(예: 직전에 5개를 골랐으면 "5개 세션 동시 진행 (직전 선택)"), 캐시가 있어도 절대 질문을 건너뛰지 않는다. 옵션은 `["3개 세션 동시 진행 (권장)", "1개(기존처럼 순차 진행)", "5개 세션 동시 진행"]` (직전 선택값이 있으면 그 옵션에 권장 표시를 옮긴다). 고른 값을 3/1/5 중 하나의 숫자로 매핑해 `save_setting(key: "parallelSessionCount", value: ...)`로 저장한다(다음 실행의 권장 라벨용). 이 값은 4단계에서 몇 개의 브라우저 탭으로 화면을 나눠 동시에 테스트할지를 결정한다 — 클수록 빠르지만 대상 서비스에 부하를 더 준다는 점을 질문 본문에 함께 안내해라.
7. 이 1단계는 최초 1회만 전부 수행한다 — 단, 6번(병렬 세션 수)은 예외로 매 실행마다 다시 묻는다. 이미 URL에 대한 자격증명과 템플릿 경로가 캐시되어 있으면, 이후 실행은 **URL만으로** 바로 2단계(사전 확인)부터 시작할 수 있어야 한다. (이슈 검수 방식 설정은 URL과 무관하게 한 번만 물어보면 계속 재사용된다.)
