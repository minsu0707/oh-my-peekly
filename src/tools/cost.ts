...
const estimateCostInput = {
  ...
  checklistItemCount: z
    ...
    .optional()
    .describe(
      `Number of checklist items assumed per screen. Provisional default: ${DEFAULT_CHECKLIST_ITEM_COUNT} ` +
...
export function registerCostTools(server: McpServer): void {
  server.registerTool(
    "estimate_cost",
    {
      ...
      description:
        "Given a screen count (typically from sitemap_crawl) and an optional checklist-item-per-screen count, " +
