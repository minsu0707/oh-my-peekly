...
export function registerCrawlerTools(server) {
    server.registerTool("sitemap_crawl", {
        title: "Crawl sitemap",
        ...
        try {
            ...
            if (!normalizedStart) {
                return textResult({ success: false, error: `Invalid or unsupported start URL: ${startUrl}` }, true);
            }
            ...
            while (queue.length > 0) {
                ...
                for (const rawLink of rawLinks) {
                    ...
                    if (!normalized)
                        continue;
                    if (new URL(normalized).origin !== startOrigin)
                        continue; // external domain: excluded per design
